        ...
        if (($handle = fopen($destination, "r")) !== FALSE) {
            ...
            while (($arr = fgetcsv($handle)) !== false) {
                ...
                if ($arr[0] != '' && $arr[2] != '' && $arr[3] != '' && $arr[4] != '' && $arr[5] != '' && $arr[6] != '' && $arr[34] != '' && $arr[35] != '' && $arr[36] != '') {
                    ...
                    if ($this->student->duplicate_check($arr[36])) {
                        
                       $max_id = $this->db->order_by('id', 'DESC')->get('guardians')->row()->id;
                       $guardian_user['email']    =  isset($arr[36]) ? $max_id.'_'.$arr[36] : '';                        
                    } 
                    ...
                    $user['phone'] = isset($arr[9]) ? $arr[9] : '';    
                     ...
                     // need to check email unique for stusents
                    $user['email'] = isset($arr[5]) ? $arr[5] : '';
                    if ($this->student->duplicate_check($arr[5])) {
                        
                       $max_id = $this->db->order_by('id', 'DESC')->get('students')->row()->id;
                       $user['email']    =  isset($arr[5]) ? $max_id.'_'.$arr[5] : '';                        
                    } 
                    $user['password'] = rand();   
