...
class Type extends MY_Controller {
    ...
    public function edit($id = null) {       
        ...
        if ($_POST) {
            ...
            if ($this->type()) {
                ...
                $this->data['type_data'] = $this->type->get_single('complain_types', array('id' => $this->input->post('id')));
                error($this->lang->line('already_exist'));
                redirect('complain/type/edit/' . $this->input->post('id'));
            }
        } else {
            if ($id) {
                $this->data['type_data'] = $this->type->get_single('complain_types', array('id' => $id));

                if (!$this->data['type_data']) {
                     redirect('complain/type/index');
                }
            }
        }
    ...
    * ********************************************************** */
    private function _prepare_type_validation() {
        ...
        $this->form_validation->set_rules('type', $this->lang->line('complain_type'). 'xss_clean|trim|required|callback_type');       
    }
