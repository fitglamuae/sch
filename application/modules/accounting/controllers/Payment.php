...
class Payment extends My_Controller {
    ...
    * ********************************************************** */
    public function ipay($data) {
        ...
        $fields = array(
                ...
                "curr"=> strtoupper($this->gsms_setting->currency),
                "p1"=> "airtel",
    ...
    * ********************************************************** */
    public function flutterwave($data) {
        ...
        $flat_data['currency'] = strtoupper($this->gsms_setting->currency);
    ...
    * ********************************************************** */
    public function ccavenue($data) {
        ...
        $this->invoice->update('invoices', array('temp_amount'=>$data['amount']), array('id'=>$data['invoice_id']));
        $pay_amount = $data['amount'];
        if($payment_setting->cca_extra_charge > 0){
            $pay_amount = $data['amount'] + ($payment_setting->cca_extra_charge/100*$data['amount']);
        } 
    ...
    * ********************************************************** */
    public function stripe($data){   
        ...
        $stripe_data = array(
            'image_url' =>  UPLOAD_PATH.'/logo/'. $this->gsms_setting->logo,
            ...
            'amount' => $pay_amount,
            'currency' => $this->gsms_setting->currency,
        );
    ...
    * ********************************************************** */
    public function sslcommerz($data){   
        ...
        $response = $this->sslcommerz->sendRequest($post_data);
        
        if($response == 'auth'){            
            error('FAILED TO CONNECT WITH SSLCOMMERZ API');
        }else if($response == 'json'){
            error('JSON Data parsing error!. Please try again.');
        }
    ...
    * ********************************************************** */
    public function pay_u_money($data) {
        ...
        $array['furl'] = base_url('accounting/payment/payumoney_success/' . $data['invoice_id']);
